...
function Header({ onLoginOpen, onScrollToContact, logo, collegeName }) {
      ...
      <div className='backdrop-blur-xs flex justify-between items-center px-3 sm:px-6 lg:px-12 py-2 mx-2 sm:mx-3 rounded-xl bg-transparent border border-white/20 shadow-sm'>
        <div className='flex items-center gap-3 sm:gap-5'>
          <a href="#" className="flex items-center gap-3">
            <img
              src={logo || brandLogo}
              alt={collegeName || "PW IOI Logo"}
              className="w-16 md:w-20 lg:w-24 max-h-12 object-contain"
            />
            {logo && <span className="text-black font-bold hidden sm:block text-sm md:text-base">{collegeName}</span>}
          </a>
        </div>
